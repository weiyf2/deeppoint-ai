...
class DouYinCrawler(AbstractCrawler):
    ...
    async def launch_browser(
        ...
        if config.SAVE_LOGIN_STATE:
            # 使用系统临时目录，避免项目路径中的中文/空格导致问题
            import tempfile
            temp_base = tempfile.gettempdir()
            user_data_dir = os.path.join(temp_base, "douyin_crawler_browser", config.USER_DATA_DIR % config.PLATFORM)  # type: ignore
