...
class CDPBrowserManager:
    ...
    async def _launch_browser(self, browser_path: str, headless: bool):
        ...
        # 注意：使用固定路径避免中文/空格导致的问题
        user_data_dir = None
        if config.SAVE_LOGIN_STATE:
            # 优先使用系统临时目录下的固定路径，避免项目路径中的特殊字符问题
            import tempfile
            temp_base = tempfile.gettempdir()  # Windows: C:\Users\xxx\AppData\Local\Temp
            user_data_dir = os.path.join(
                temp_base,
                "douyin_crawler_browser",
