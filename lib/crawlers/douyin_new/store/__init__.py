...
class DouyinStoreFactory:
    ...
    def create_store() -> AbstractStore:
        ...
        if not store_class:
            raise ValueError("[DouyinStoreFactory.create_store] Invalid save option only supported csv or json or excel ...")
        return store_class()
...
async def update_douyin_aweme(aweme_item: Dict):
    ...
    save_content_item = {
        ...
        "user_id": anonymize_user_id(user_info.get("uid")),
        "sec_uid": "",
        "short_user_id": "",
        "user_unique_id": "",
        "user_signature": "",
    ...
    utils.logger.info(f"[store.update_douyin_aweme] douyin aweme id:{aweme_id}, title:{save_content_item.get('title')}")
...
async def update_dy_aweme_comment(aweme_id: str, comment_item: Dict):
    ...
    if aweme_id != comment_aweme_id:
        utils.logger.error(f"[store.update_dy_aweme_comment] comment_aweme_id: {comment_aweme_id} != aweme_id: {aweme_id}")
        return
    ...
    utils.logger.info(f"[store.update_dy_aweme_comment] douyin aweme comment: {comment_id}, content: {save_comment_item.get('content')}")

    await DouyinStoreFactory.create_store().store_comment(comment_item=save_comment_item)


async def save_creator(user_id: str, creator: Dict):
    utils.logger.info("[store.save_creator] creator profile persistence skipped for privacy")
    return
